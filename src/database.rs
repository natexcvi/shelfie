...
const DB_NAME: &str = ".fs_organizer.db";
...
impl Database {
    ...
    fn initialize_schema(&mut self) -> Result<()> {
        self.conn.execute_batch(
            ...
            CREATE TABLE IF NOT EXISTS items (
                ...
                needs_content_read BOOLEAN NOT NULL DEFAULT 0,
            ...
            CREATE INDEX IF NOT EXISTS idx_items_needs_content ON items(needs_content_read);
            "
        )?;
        Ok(())
    }
    ...
    pub fn get_cabinet_by_name(&self, name: &str) -> Result<Option<Cabinet>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, description, created_at FROM cabinets WHERE name = ?1"
        )?;
    ...
    pub fn list_cabinets(&self) -> Result<Vec<Cabinet>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, description, created_at FROM cabinets ORDER BY name"
        )?;
        
        let cabinets = stmt.query_map([], |row| {
            Ok(Cabinet {
                id: row.get(0)?,
                name: row.get(1)?,
                description: row.get(2)?,
    ...
    pub fn list_shelves(&self, cabinet_id: Option<i64>) -> Result<Vec<Shelf>> {
        let query = if let Some(cabinet_id) = cabinet_id {
            format!("SELECT id, cabinet_id, name, description, created_at FROM shelves 
                    WHERE cabinet_id = {} ORDER BY name", cabinet_id)
        } else {
        ...
        let shelves = stmt.query_map([], |row| {
            Ok(Shelf {
                id: row.get(0)?,
                cabinet_id: row.get(1)?,
                name: row.get(2)?,
    ...
    pub fn insert_item(&self, item: &Item) -> Result<i64> {
        ...
        self.conn.execute(
            "INSERT INTO items (shelf_id, path, original_name, suggested_name, description, 
                              file_type, is_opaque_dir, needs_content_read, processed_at) 
    ...
    pub fn update_item_content(&self, item_id: i64, description: &str, suggested_name: &str) -> Result<()> {
        self.conn.execute(
    ...
    pub fn get_processing_state(&self, key: &str) -> Result<Option<String>> {
        let mut stmt = self.conn.prepare(
            "SELECT value FROM processing_state WHERE key = ?1"
        )?;
